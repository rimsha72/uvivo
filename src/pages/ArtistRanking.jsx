...
export default function ArtistRanking() {
    ...
    const handleCommissionChange = (rank, value) => {
        setCommissionRates(prev => ({ ...prev, [rank]: value }));
    };
        ...
        <div>
            ...
            <div className="max-w-screen-3xl mx-auto">
                <div className="mx-4 sm:mx-8 my-4 space-y-4 fnt-sans">
                    <div className="border border-gray-750 rounded-lg pt-7 pb-3 drop-shadow-sm bg-white mt-8">
                        <div className="flex lg:flex-row flex-col gap-4 lg:gap-6 items-center justify-between mb-4 w-full lg:px-7 px-4">
                        ...
                        <div className='px-7 py-3 mb-4 flex justify-end'>
                        <button onClick={() => setIsRankingOpen(true)} className="bg-red-150 text-white py-2 px-4 rounded-lg text-sm">Manage Rankings</button>
                        </div>
                        <Ranking />
             ...
             {isRankingOpen && (
                <div className="overflow-y-auto overflow-x-hidden font-sans fixed flex bg-black/20 backdrop-blur-xs z-50 justify-center items-center w-full md:inset-0 max-h-full">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        {Object.keys(commissionRates).map((rank) => (
                            <div key={rank} className="mb-4">
                                <label className="block text-gray-150 capitalize text-sm font-medium mb-1">{rank} (%)</label>
                                <input 
                                    type="number" 
                                    value={commissionRates[rank]} 
                                    onChange={(e) => handleCommissionChange(rank, e.target.value)} 
                        ...
                        <div className="flex justify-end mt-6">
                            <button onClick={() => setIsRankingOpen(false)} className="border border-red-150 bg-white text-red-150 py-2 px-5 rounded-lg mr-2 cursor-pointer">Close</button>
                            <button onClick={() => setIsRankingOpen(false)} className="bg-red-150 text-white py-2 px-5 rounded-lg cursor-pointer">Save</button>
                        </div>
                    </div>
                </div>
            )}
